      ...
      <slot name="body">
        ...
        <template v-if="type === '1'">
          <a :href="getUrl(item.url)" v-for="item in list" @click.prevent="onItemClick(item)" class="weui-media-box weui-media-box_appmsg">
            <div class="weui-media-box__hd" v-if="image === 'icon' && item.icon">
              <i class="weui-media-box__thumb weui-icon_msg" :class="getClass(item.icon)" :style="getStyle(item.icon)"></i>
            </div>
            <div class="weui-media-box__hd" v-else-if="item.src">
          ...
          <div class="weui-media-box weui-media-box_small-appmsg">
            <div class="weui-cells">
              <a class="weui-cell weui-cell_access" :href="getUrl(item.url)" v-for="item in list" @click.prevent="onItemClick(item)">
                <div class="weui-media-box__hd" v-if="image === 'icon' && item.icon">
                  <i class="weui-media-box__thumb weui-icon_msg" :class="getClass(item.icon)" style="font-size:60px;"></i>
                </div>
                <div class="weui-media-box__hd" v-else-if="item.src">
...
export default {
  ...
  methods: {
    ...
    getClass (icon) {
      return `weui-icon-${icon.type || 'success'}`
    },
    getStyle (icon) {
      let iconstyle = `vertical-align: middle;font-size: ${icon.size || '60px'};`
      if (icon && icon.color) {
...
@import '../../node_modules/vux/src/styles/weui/icon/weui_font.less';
@import '../../node_modules/vux/src/styles/weui/icon/weui_icon_font.less';
</style>
